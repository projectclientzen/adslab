...
export function CampaignTable() {
  ...
  const { campaigns, isLoading, isLive, refetch } = useDashboardData(activeBrand, dateRange);
  ...
  useEffect(() => {
    const saved = localStorage.getItem(ORDER_KEY(activeBrand));
    if (saved) {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    setOrderedIds(ids => {
      ...
      localStorage.setItem(ORDER_KEY(activeBrand), JSON.stringify(next));
      return next;
    });
  };
