...
import { useApp, useActiveBrand } from '@/context/AppContext';
import { useDashboardData, useAllFetchStatuses } from '@/hooks/useSupabaseData';
import { BRANDS, MOCK_ALERTS } from '@/lib/mockData';
import { triggerMetaFetch } from '@/lib/queries';
import { cn } from '@/lib/utils';
import { TimeAgo } from '@/components/TimeAgo';
import type { BrandId, DateRange } from '@/lib/types';
...
export function Header() {
  const { activeBrand, setActiveBrand, dateRange, setDateRange, role, setRole, isLiveData } = useApp();
  const { refetch, isLoading, alerts } = useDashboardData(activeBrand, dateRange);
  const allStatuses = useAllFetchStatuses();
  const [showAlerts, setShowAlerts] = useState(false);
  const [fetching, setFetching] = useState(false);

  const brandStatus = allStatuses.find(s => s.brandId === activeBrand);
  const unreadAlerts = alerts.filter(a => !a.isRead);

  const handleForceRefetch = async () => {
    setFetching(true);
    await triggerMetaFetch(activeBrand);
    ...
    <header className="min-h-14 border-b border-border bg-surface flex items-center flex-wrap px-3 sm:px-4 gap-2 sm:gap-4 flex-shrink-0 py-2">
      ...
      <div className="flex items-center gap-1 bg-card rounded-lg p-1 border border-border">
        {BRANDS.map(b => (
          <button
            key={b.id}
            onClick={() => setActiveBrand(b.id as BrandId)}
            className={cn(
              'px-3 py-1.5 rounded-md text-xs font-medium transition-all',
              activeBrand === b.id ? 'text-white shadow-sm' : 'text-text-secondary hover:text-text-primary',
            )}
      ...
      <button
        ...
        className={cn(
          ...
          role === 'admin'
            ? 'bg-primary-dim border-primary/30 text-primary'
            : 'bg-border border-border-light text-text-secondary',
        )}
      >
            ...
            <div className="max-h-80 overflow-y-auto">
              {alerts.map(alert => (
                <div key={alert.id} className={cn(
                  'p-3 border-b border-border last:border-0 transition-colors hover:bg-border/40',
                  !alert.isRead && 'bg-surface',
                )}>
                  <div className="flex items-start gap-2">
                    ...
                    <div className="min-w-0">
                      ...
                      {alert.suggestAction && (
                        <p className="text-[11px] text-primary mt-1">→ {alert.suggestAction}</p>
                      )}
              ...
              {alerts.length === 0 && (
                <p className="p-4 text-center text-xs text-text-muted">Tidak ada alert aktif</p>
              )}
            </div>
          </div>
        )}
